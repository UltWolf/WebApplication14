...
namespace WebApplication14
{
    public class NVPAPICaller
    {
        ...
        public bool ShortcutExpressCheckout(string amt, ref string token, ref string retMsg,List<Order> orderList)
        {
            if (bSandbox)
            {
            ...
            List<Order> myOrderList = new List<Order>(orderList);

                for (int i = 0; i < myOrderList.Count; i++)
                {
                    encoder["L_PAYMENTREQUEST_0_NAME" + i] = myOrderList[i].Product.Name.ToString();
                    encoder["L_PAYMENTREQUEST_0_AMT" + i] = myOrderList[i].Product.Price.ToString();
